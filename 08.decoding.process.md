...
When AV1 film grain metadata is not present in the bitstream, the AV1 film grain synthesis module shall produce output pictures that are identical in all respects
and have the same output order as the input pictures to the AV1 film grain synthesis module.

When AV1 film grain metadata is present in the bitstream, a film grain synthesis module shall implement a film grain synthesis process that
modifies the input arrays OutY, OutU, OutV in a manner similar to what is specified in this specification. The reference film grain synthesis process is described in [section 8.2][].
...
following two options:
  ...
  2. A conformant AV1 film grain synthesis module shall produce output pictures that are in the same order and do not have perceptually
...
The input to this process is a sequence of the reconstructed video pictures and a sequence of AV1 film grain metadata messages.
...
This process is invoked to process video pictures.
...
intermediate values of video pictures from the previous post-processing modules.
