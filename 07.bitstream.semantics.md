...
(v) bitdepth equal to the bitdepth of the decoded frame when the bitdepth syntax element is present, 
(vi) color_primaries equal to the color primaries of the decoded frame when the color_primaries syntax element is present, 
(vii) transfer_characteristics equal to the transfer characteristics of the decoded frame when the transfer_characteristics syntax element is present, 
(viii) matrix_coefficients equal to the matrix coefficients of the decoded frame when the matrix_coefficients syntax element is present, and
(ix) video_full_range_flag equal to the video full range flag of the decoded frame when the video_full_range_flag syntax element is present.
...
bytes may be useful for applications that choose to create an av1_film_grain_payload prior to determining the 
payload contents.
{:.alert .alert-info }
...
current film grain parameters.  The integer value is defined by the “Color primaries” section of ISO/IEC 23091-4 and/or
...
the decoded video frame that is associated with the current film grain parameters.  The behavior of the film
grain synthesis process in response to the syntax element not matching the decoded frame associated with the current
...
with the current film grain parameters.  The integer value is defined by the “Transfer characteristics” section of 
ISO/IEC 23091-4 and/or ITU-T H.273.  The syntax element is intended to be informative and is not used in the film grain 
process defined in this specification.  When present, transfer_characteristics should match any transform characteristic
information of the decoded video frame that is associated with the current film grain parameters.  The behavior of the film
grain synthesis process in response to the syntax element not matching the decoded frame associated with the current
film grain parameters is outside the scope of this specification.

**matrix_coefficients** is an integer that indicates the matrix coefficients of the decoded frame associated with the
current film grain parameters.  The integer value is defined by the “Matrix coefficients” section of ISO/IEC 23091-4 
...
of the decoded video frame that is associated with the current film grain parameters.  The behavior of the film
grain synthesis process in response to the syntax element not matching the decoded frame associated with the current
film grain parameters is outside the scope of this specification.

**video_full_range_flag** is a flag that indicates the scaling and offset values applied in association with the matrix 
coefficients of the decoded frame associated with the current film grain parameters.  The value is defined by the 
“Matrix coefficients” section of ISO/IEC 23091-4 and/or ITU-T H.273.  The syntax element is intended to be informative 
and is not used in the film grain process defined in this specification.  When present, video_full_range_flag should 
match any video full range information of the decoded video frame that is associated with the current film grain parameters.  
The behavior of the film grain synthesis process in response to the syntax element not matching the decoded frame 
...
When predict_y_scaling_flag parameters is equal to 1, the luma scaling function values are derived as follows
...
It is a requirement of the specification conformance that num_y_points is less than or equal to 14.
...
Values of
point_cb_value_ref[ i ] are equal to the values point_cb_value[ i ] in the film grain parameters set referenced by
film_grain_param_set_idx_for_prediction. Values of point_cb_scaling_ref[ i ] are equal to
point_cb_scaling[ i ] in the film grain parameters set referenced by film_grain_param_set_idx_for_prediction.
...
Values of point_cr_value_ref [i] are equal to the values of point_cr_value [i] in the film grain parameters set
referenced by film_grain_param_set_idx_for_prediction. Values of point_cr_scaling_ref [i] are equal to
the values of point_cr_scaling [i] in the film grain parameters set referenced by film_grain_param_set_idx_for_prediction.
