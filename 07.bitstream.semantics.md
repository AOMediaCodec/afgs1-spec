...
synthesis. The film grain process can be applied to pictures output by a video decoder or
...
**Note:** In the case of update_grain_flag being equal to 0, the film grain synthesis module should use the parameters
...
specifies that the previous set of parameters in the picture display order should be used.

It is a requirement of conformance to this specification that for every distinct value of film_grain_param_set_idx used in a coded video
sequence, the first picture in the display order with that value of film_grain_param_set_idx that has apply_grain_flag flag equal to 1
...
film grain parameters is outside the scope of this specification. When color_primaries is not present its value shall 
...
grain synthesis process in response to this syntax element not matching the decoded picture associated with the current
film grain parameters is outside the scope of this specification. When transfer_characteristics is not present its value shall 
...
grain synthesis process in response to this syntax element not matching the decoded picture associated with the current
film grain parameters is outside the scope of this specification. When matrix_coefficients is not present its value shall 
...
not present its value shall be set to be equal to the corresponding video full range flag value of its associated decoded video picture.
...
parameters num_y_points in the film grain parameter set film_grain_param_set_idx_for_prediction. Values
point_y_value_ref[ i ] are equal to point_y_value[ i ] in the film grain parameter set
film_grain_param_set_idx_for_prediction. Values of point_y_scaling_ref[ i ] are equal to point_y_scaling[ i ]
in the film grain parameter set film_grain_param_set_idx_for_prediction.
...
to num_cb_points in the film grain parameter set with film_grain_param_set_idx equal to film_grain_param_set_idx_for_prediction. 
The values of point_cb_value_ref[ i ] and point_cb_scaling_ref[ i ] for all instances of index i are equal to point_cb_value[ i ] and point_cb_scaling[ i ], respectively, in the film grain parameter set with film_grain_param_set_idx equal to film_grain_param_set_idx_for_prediction. 
...
**Note:** cb_scaling_mult represents a value of a multiplier from -16 to +15.9375 with a value granularity of 1/16. cb_scaling_add represents an integer value of intercept in the range between [ - 256, 255].
{:.alert .alert-info }
...
It is a requirement of conformance to this specification that the parameter num_cr_points_in_ref is set equal
to num_cr_points in the film grain parameter set with film_grain_param_set_idx equal to film_grain_param_set_idx_for_prediction.
The values of point_cr_value_ref[ i ] and point_cr_scaling_ref[ i ] for all instances of index i are equal to point_cr_value[ i ] and point_cr_scaling[ i ], respectively, in the film grain parameter set with film_grain_param_set_idx equal to film_grain_param_set_idx_for_prediction. 
