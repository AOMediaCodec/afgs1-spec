...
For a given decoded picture, if film grain parameter sets are transmitted
for this picture, one film grain parameter set with apply_horz_resolution and
...
**Note:** The conformance statements above mean that if film grain parameter sets are transmitted
for a decoded picture, there must be one film grain parameter set that corresponds to the
