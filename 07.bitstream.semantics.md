...
**video_signal_characteristics_flag** equal to 1 specifies that the syntax element bit_depth_minus8 is present in this message.
video_signal_characteristics_flag equal to 0 specifies that the syntax element bit_depth_minus8 is not present in this message.
...
within the current metadata_aom_itut_t35( ).

**Note:** This requirement restricts prediction between the film grain parameters sets to be within one 
metadata_aom_itut_t35( ).  It also restricts the reference for the prediction process to be the film grain parameter set 
