...
If for any film grain parameter set cicp_info_present_flag is set to 1, then cicp_info_present_flag should be set for all film grain parameter sets present in a bitstream.

If cicp_info_present_flag is set to 1 for any film grain parameter set, then, for a given decoded picture, there shall not be more than one film grain parameter set for a given combination of luma horizontal, luma vertical resolution, color_primaries, transfer_characteristics, matrix_coefficients, and video_full_range_flag.

Otherwise, if cicp_info_present_flag is not set to 1 for any film grain parameter set, then, for a given decoded picture, there shall not be more than one film grain parameter set for a given combination of luma horizontal and luma vertical resolution.
...
that produce different resolutions) and film grain parameter sets are present, then
...
processing the syntax elements parsed from the syntax tables.
