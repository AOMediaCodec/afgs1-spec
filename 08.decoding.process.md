[AMT]: # (These conformance requirements seem not appropriate and seem to relate mostly to AV1, not the film grain generation process. I think the text here should be removed or severely modified).
...
[AMT]: # (From where is BitDepth derived? IT should not depend on the content and this needs to be independent. Therefore this needs to be signalled. Note that some specs may use different luma/chroma bitdepth and having this being independent also helps avoid confusion. )
...
[AMT]: # (Should this be "This process computes a lookup table for each color component available"?)
