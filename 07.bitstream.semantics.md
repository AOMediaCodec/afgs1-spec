...
picture >> apply_units_resolution_log2 and (ii) the vertical resolution of the coded
picture >> apply_units_resolution_log2 shall be provided.
...
**update_grain** equal to 1 means that a new set of parameters is sent for the film_grain_param_set_idx.
update_grain equal to 0 means that the previous set of parameters with index film_grain_param_set_idx should be used.
...
**video_signal_characteristics_flag** equal to 1 indicates that the syntax elements bit_depth, mono_chrome, subsampling_x, subsampling_y are signaled in this metadata message.
video_signal_characteristics_flag equal to 0 indicates that the syntax elements bit_depth, mono_chrome, subsampling_x, subsampling_y are not signal in this metadata message.
If video_signal_characteristics_flag equal to 0, variables BitDepth, mono_chrome, SubX, and SubY are derived from the decoded video picture, to which this metadata message
belongs. 

**bit_depth** is a syntax element which determines bit depth of the decoded video sequence or a decoded video picture, to which this metadata belongs to.
