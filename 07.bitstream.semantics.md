...
**mono_chrome** equal to 1 specifies that the film grain does not contain U and V color planes. mono_chrome equal to 0 
specifies that the film grain contains Y, U, and V color planes.

**subsampling_x**, **subsampling_y** specify the chroma subsampling format of the film grain:
...
**video_frame_characteristics_flag** equal to 1 specifies that the syntax element bit_depth is signaled in this metadata message.
video_frame_characteristics_flag equal to 0 specifies that the syntax element bit_depth is not signaled in this metadata message.
When video_frame_characteristics_flag equal to 0, the variable BitDepth is derived from the decoded video frame that is associated
