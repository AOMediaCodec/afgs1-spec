...
[AMT]: # (Need to consider how layers are handled)
...
[AMT]: # (This seems incorrect since in that case it seems that we are loading the parameters based on the set_idx, not on the previous frame)
...
[AMT]: # (What happens if a frame does not have parameters associated with it?)
...
[AMT]: # (See comment also about resolution.)
{:.alert .alert-info }
...
[AMT]: # (Where are the elements subx and suby? SEI messages should not depend on external information)
...
[AMT]: # (Should this be a "shall" or a "should". My understanding is that the impact should be minor if one does not clip. Also, if 0, why even discuss clipping to full range? )
