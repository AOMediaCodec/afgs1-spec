...
This metadata provides the decoder or post-processing module with a model for parameterized film grain
...
**film_grain_model_persistence_flag** specifies the persistence of the AV1 film grain characteristics model for the current reconstruted
video sequence.
film_grain_model_persistence_flag equal to 0 specifies that the model specified in AV1 film grain characteristics SEI message applies to the current decoded frame only.

Let frame FrameA be the current frame. film_grain_model_persistence_flag equal to 1 specifies that the AV1 film grain characteristics metadata model persists for the current coded video sequence in output order until any of the following conditions are true:
  * A new coded video sequence begins.
  * The bitstream ends.
  * A frame FrameB in an access unit containing AV1 film grain characteristics metadata that is applicable to the coded video sequence is output for which output picture order identifier is greater than output picture order identifier for frameA.
...
It is a requirement of the bitstream conformance that if the associated frame belongs to a so-called Random Access Point (RAP), the value of update_grain shall be equal to 1.
